CLASS zcl_gtt_pof_pe_filler_dli DEFINITION
  PUBLIC
  CREATE PUBLIC .
...
CLASS ZCL_GTT_POF_PE_FILLER_DLI IMPLEMENTATION.
  ...
  METHOD add_packing_event.
    ...
    IF io_relevance->is_enabled(
         iv_milestone   = zif_gtt_pof_app_constants=>cs_milestone-dl_packing ) = abap_true.
      ...
      ct_expeventdata = VALUE #( BASE ct_expeventdata (
        ...
        language          = sy-langu
        appobjid          = is_app_objects-appobjid
        milestone         = zif_gtt_pof_app_constants=>cs_milestone-dl_packing
  ...
  METHOD add_put_away_event.
    ...
    IF io_relevance->is_enabled(
         iv_milestone   = zif_gtt_pof_app_constants=>cs_milestone-dl_put_away ) = abap_true.
      DATA(lv_tzonrc) = zcl_gtt_pof_tools=>get_field_of_structure(
              ir_struct_data = is_app_objects-mastertabref
              iv_field_name  = 'TZONRC' ).
      ...
      ct_expeventdata = VALUE #( BASE ct_expeventdata (
        ...
        language          = sy-langu
        appobjid          = is_app_objects-appobjid
        milestone         = zif_gtt_pof_app_constants=>cs_milestone-dl_put_away
        evt_exp_tzone     = COND #( WHEN lv_tzonrc IS NOT INITIAL
                                             THEN lv_tzonrc
                                             ELSE zcl_gtt_pof_tools=>get_system_time_zone( ) )
        evt_exp_datetime  = zcl_gtt_pof_dl_tools=>get_delivery_date(
  ...
  METHOD zif_gtt_pof_pe_filler~check_relevance.
    ...
    IF zcl_gtt_pof_dl_tools=>is_appropriate_dl_type( ir_struct = is_app_objects-mastertabref ) = abap_true AND
       zcl_gtt_pof_dl_tools=>is_appropriate_dl_item( ir_struct = is_app_objects-maintabref ) = abap_true.
      ...
      ELSE.
        DATA(lo_relevance_old)  = NEW zcl_gtt_pof_event_rel_dl_it(
                                        io_ef_parameters = mo_ef_parameters ).

        DATA(lo_relevance_new)  = NEW zcl_gtt_pof_event_rel_dl_it(
                                        io_ef_parameters = mo_ef_parameters
                                        is_app_objects   = is_app_objects ).

        DATA(lt_milestones)     = VALUE tt_milestones(
          ( zif_gtt_pof_app_constants=>cs_milestone-dl_goods_receipt )
          ( zif_gtt_pof_app_constants=>cs_milestone-dl_packing )
          ( zif_gtt_pof_app_constants=>cs_milestone-dl_put_away )
        ).
  ...
  METHOD zif_gtt_pof_pe_filler~get_planed_events.
    ...
    DATA(lo_relevance)  = NEW zcl_gtt_pof_event_rel_dl_it(
                            io_ef_parameters = mo_ef_parameters
                            is_app_objects   = is_app_objects ).
