FUNCTION ZPOF_GTT_OTE_DL_HDR_REL .
*"----------------------------------------------------------------------
*"*"Local Interface:
*"  IMPORTING
...
*"----------------------------------------------------------------------
  ...
  TRY.
      e_result  = zcl_gtt_pof_ef_performer=>check_relevance(
                    is_definition         = VALUE #( maintab = zif_gtt_pof_app_constants=>cs_tabledef-dl_header_new )
                    io_bo_factory         = NEW zcl_gtt_pof_tp_factory_dlh( )
                    iv_appsys             = i_appsys
