FUNCTION ZPOF_GTT_EE_DL_HDR_GR .
*"----------------------------------------------------------------------
*"*"Local Interface:
*"  IMPORTING
 ...
 DATA: lo_udm_message    TYPE REF TO cx_udm_message,
  ...
  TRY.
      zcl_gtt_pof_ae_performer=>get_event_data(
        EXPORTING
          is_definition           = VALUE #( maintab   = zif_gtt_pof_app_constants=>cs_tabledef-md_material_header )
          io_ae_factory           = NEW zcl_gtt_pof_ae_factory_dlh_gr( )
          iv_appsys               = i_appsys
