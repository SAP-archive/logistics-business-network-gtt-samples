...
@Service public class LocationServiceImpl implements LocationService {
    ...
    @Override public List<Location> getLocationsWithLocationAltKeys(@NotNull Set<String> locationAltKeys) {
        return gttCoreServiceClient.getLocations(locationAltKeys);
    }

    @Override public void fillLocations(List<TimelineEvent> timelineEvents) {
        Set<String> locationAltKeys =
            timelineEvents.stream().filter(timelineEvent -> nonNull(timelineEvent.getLocationAltKey()))
                .map(TimelineEvent::getLocationAltKey).collect(toSet());
