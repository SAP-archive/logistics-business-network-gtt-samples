...
public abstract class AbstractEventService {
    protected static final String GOODS_RECEIPT_URI = "/GoodsReceipt";
    ...
    protected final GTTCoreServiceClient gttCoreServiceClient;

    protected AbstractEventService(GTTCoreServiceClient gttCoreServiceClient) {
    ...
    protected <T> List<T> queryAllEvents(String uriTemplate, String filterPart, String delimiter, List<Object> args, Class<T> clazz) {
        String[] split = uriTemplate.split("\\?");
        String httpUrl = split[0];
        String initialQuery = split.length > 1 ? split[1] : FILTER + "=";
    ...
    protected boolean isDeletionLatestEvent(UUID processId) {
        ...
        Optional<Event> max = eventDirectories.stream()
                ...
                .filter(event -> nonNull(event.getEventType())
                        && (event.getEventType().endsWith(DELETION_EVENT_ENTITY_NAME) || event.getEventType().endsWith(UNDELETION_EVENT_ENTITY_NAME)))
